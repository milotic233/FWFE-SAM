...
def analyze_tensor_parts_difference(tensor_data):
    """
    分析 shape 为 [N, M, 2] (例如 [576, 32, 2]) 的 Tensor 中，
    最后维度的两个部分 (part_0 和 part_1) 的区别。
    """
    # 0. 形状检查
    if tensor_data.dim() != 3 or tensor_data.shape[-1] != 2:
        print(f"Error: Expected shape [N, M, 2], but got {tensor_data.shape}")
        return

    print(f"--- Analyzing Tensor Split (Shape: {tensor_data.shape}) ---")

    # 1. 切片分离 (Slicing)
    # 取出最后一个维度的第0个部分 -> [576, 32]
    part_0 = tensor_data[:, :, 0]
    # 取出最后一个维度的第1个部分 -> [576, 32]
    part_1 = tensor_data[:, :, 1]

    print(f"Part 0 Shape: {part_0.shape}")
    print(f"Part 1 Shape: {part_1.shape}")

    # 2. 检查是否完全相等 (Exact Match)
    if torch.equal(part_0, part_1):
        print("\n✅ Result: Both parts are EXACTLY identical.")
        return
    else:
        print("\n⚠️ Result: Parts are DIFFERENT.")

    # 3. 计算差异统计 (Statistical Difference)
    # 计算差值 tensor
    diff_tensor = part_0 - part_1
    abs_diff = torch.abs(diff_tensor)
    
    max_diff = torch.max(abs_diff).item()
    mean_diff = torch.mean(abs_diff).item()
    mse = torch.nn.functional.mse_loss(part_0, part_1).item()

    print(f"\n[Statistics]:")
    print(f"  Max Absolute Diff:  {max_diff:.6f}")
    print(f"  Mean Absolute Diff: {mean_diff:.6f}")
    print(f"  MSE (L2 Loss):      {mse:.6f}")

    # 4. 定位最大差异位置 (Locate Max Difference)
    # argmax 返回扁平化后的索引，需要 unravel 找回 [row, col]
    flat_idx = torch.argmax(abs_diff)
    row_idx = (flat_idx // part_0.shape[1]).item()
    col_idx = (flat_idx % part_0.shape[1]).item()
    
    val_0 = part_0[row_idx, col_idx].item()
    val_1 = part_1[row_idx, col_idx].item()

    print(f"\n[Largest Discrepancy Found]:")
    print(f"  At index: [{row_idx}, {col_idx}]")
    print(f"  Part 0 Value: {val_0:.6f}")
    print(f"  Part 1 Value: {val_1:.6f}")
    print(f"  Difference:   {abs(val_0 - val_1):.6f}")

    # 5. (可选) 余弦相似度 - 检查它们是否方向一致但幅度不同
    # 将 [576, 32] 拉平成 [18432] 向量计算相似度
    cos_sim = torch.nn.functional.cosine_similarity(
        part_0.reshape(1, -1), 
        part_1.reshape(1, -1)
    ).item()
    print(f"\n[Structure Similarity]:")
    print(f"  Cosine Similarity: {cos_sim:.6f} (1.0 means identical direction)")
...
def reshape_for_broadcast(freqs_cis: torch.Tensor, x: torch.Tensor) -> torch.Tensor:
    ...
    if freqs_cis is not None and (not torch.is_complex(freqs_cis)) and freqs_cis.shape[-1] == 2:
...
class Attention(nn.Module):
    ...
    def forward(self, x: Tensor) -> Tensor:
        # print(self.freqs_cis.dtype, torch.is_complex(self.freqs_cis), self.freqs_cis.shape)
