...
def _create_vision_backbone(
    ...
    # Position encoding
    position_encoding = _create_position_encoding(precompute_resolution=504)
    # ViT backbone
...
def _load_checkpoint(model, checkpoint_path):
    ...
    for k, v in sam3_image_ckpt.items():
        # 1) 直接跳过 rope buffer
        # if ".7.attn.freqs_cis" in k:
        #     continue
        # if ".15.attn.freqs_cis" in k:
        #     continue
        # if ".23.attn.freqs_cis" in k:
        #     continue
        # if ".31.attn.freqs_cis" in k:
        #     continue

        # # 2) 额外保险：跳过所有 shape 不匹配（避免别的 key 也报错）
